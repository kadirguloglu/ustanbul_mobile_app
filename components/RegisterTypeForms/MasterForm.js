...
class MasterForm extends Component {
  ...
  validate = () => {
    ...
    if (model.CompanyName.length < 3) {
      Toast.show({
        text: "Firma adı alanı en az 3 karakter olmalıdır.",
        buttonText: "Tamam",
        type: "danger",
        duration: 2500
      });
    ...
    if (model.CompanyName.length > 150) {
      Toast.show({
        text: "Firma adı alanı için en fazla 150 karakter girebilirisiniz.",
        buttonText: "Tamam",
        type: "danger",
        duration: 2500
      });
        ...
        if (payload) {
          if (payload.data !== undefined && payload.data !== null) {
            if (payload.data == "1") {
              ...
              masterRegisterPost(model).then(y => {
  ...
  render() {
    ...
    const { Name, Surname, Email, Phone, Password, CompanyName } = model;
    const { customerServiceResponse, generalServiceGetResponse } = this.props;
    const {
      masterRegisterPostLoading,
          ...
          >
            ...
            <View style={styles.inputContainer}>
              ...
              <TextInput
                ...
                onChangeText={v =>
                  this.setState({ model: { ...model, CompanyName: v } })
                }
                value={CompanyName}
              />
            </View>
            ...
            {masterRegisterPostLoading || emailValidatePostLoading ? (
...
const mapStateToProps = ({
  customerServiceResponse,
  generalServiceGetResponse
}) => ({ customerServiceResponse, generalServiceGetResponse });
