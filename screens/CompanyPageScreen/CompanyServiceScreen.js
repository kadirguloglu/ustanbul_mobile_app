...
import { StyleSheet, ListView, AsyncStorage, View, Text, ScrollView } from 'react-native';
import { connect } from 'react-redux';
import { companyServicePreviewData, servicePreviewDetailData, servicePreviewDetailQuestionData, proposalDetailData, updateServiceProposal } from '../reducer-functions';
import {
                    ...
                    return (<View key={"Service" + page} style={{
                        ...
                        <View style={{ backgroundColor: 'white', margin: 10, borderRadius: 5, padding: 10 }}>
                            ...
                            }
                            {
                                (item.Key.indexOf('1#') > -1) ?
                                    (
                                        <Button full style={styles.buttonStyle} onPress={() => this.handlerUpdateServiceProposal(item, data)}>
                                            <Text style={styles.iconText}>Teklifi Güncelle</Text>
                                        </Button>
                                    ) :
                                    (item.Key.indexOf('2#') > -1) ?
                                        (
                                            <Button full style={styles.buttonStyle} onPress={() => this.handlerServiceApproved(item, data)}>
...
const mapStateToProps = ({ companyServicePreviewLoading, servicePreviewListResult, activeUser,
    ...
    updateServiceProposalLoading, updateServiceProposalResult }) => ({
...
export default connect(mapStateToProps, mapDispatchToProps)(CompanyServiceScreen);
